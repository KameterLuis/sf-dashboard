...
export default function UploadKontoauszug() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch("/api/upload-bank", { method: "POST", body });
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Upload a bank statement File</CardTitle>
        <CardDescription>
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="grid gap-4">
          <div className="flex items-center justify-center w-full">
            <label
              htmlFor="dropzone-bankstatement-file"
            ...
            >
              ...
              <input
                id="dropzone-bankstatement-file"
                type="file"
                className="hidden"
                onChange={handleFileChange}
                accept=".csv,text/csv,application/vnd.ms-excel,.CSV"
              />
            </label>
          </div>
          {file && (
